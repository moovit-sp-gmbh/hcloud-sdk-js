...
export interface Share {
    ...
    items?: (
        ...
        | { _id: string; name: string; type: ItemType.MEDIA_ASSET; thumbnailUrl: string; storageId: string }
        | { _id: string; name: string; type: ItemType.STACK; thumbnailUrl: string; storageId: string }
    )[];
    permissions?: string[];
 ...
 */
export interface ShareCreate extends Omit<
    Share,
    "_id" | "createDate" | "creator" | "permissionGroups" | "users" | "items" | "waiting" | "invalidLink"
> {
    password?: string;
    expires?: number;
    namespaces?: Record<string, string[]>;
