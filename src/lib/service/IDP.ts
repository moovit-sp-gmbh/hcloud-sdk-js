...
export default class IDP extends base {
     ...
     * @returns User object
     */
    authorize = async (token: string): Promise<User> => {
        const resp = await axios.post<User>(this.getEndpoint("/v1/authenticate"), {}, { headers: { Authorization: token } }).catch((err: Error) => {
            throw err;
        });
