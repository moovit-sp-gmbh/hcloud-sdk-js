...
export class High5Node extends base {
     ...
     * @response Node
     */
    public async createNode(nodeCategory: NodeCategory, specification: object, typescript: string, streamId: string): Promise<Node> {
     ...
     * @response Node
     */
    public async patchNode(nodeId: string, specification: object, typescript: string): Promise<Node> {
        const resp = await axios
            .patch<Node>(this.getEndpoint(`/v1/node/${nodeId}`), {
                specification: specification,
                typescript: typescript,
            })
            .catch((err: Error) => {
